...
class InstantiationService {
    ...
    invokeFunction(fn, ...args) {
        ...
        try {
            const accessor = {
                get: (id, isOptional) => {
                    if (_done) {
                        throw common_1.illegalState('service accessor is only valid during the invocation of its target method');
                    }
    ...
    _createServiceInstance(ctor, args = [], _supportsDelayedInstantiation, _trace) {
        if (!_supportsDelayedInstantiation) {
            ...
            const idle = new common_1.IdleValue(() => this._createInstance(ctor, args, _trace));
